...
## Step 4
Added hand card click selection for the Stage 5 UI. `HandCardView` now listens for left-click pointer events and reports its hand index to `GameUIController`, which forwards clicks only while the UI is in the `PlayingBlind` state. `PrototypeBootstrap` subscribes to those click events and reuses the existing `HandManager.ToggleCardSelection()` logic so UI clicks and keyboard selection update the same real card objects. `HandManager` now enforces a maximum of 5 selected cards, and the hand UI refreshes immediately after a click so the selected-card visual remains synchronized. This step does not bind Play, Discard, Sort, shop, CashOut, tooltip, or status panel UI actions.

## Step 5
Bound the core Play-state action buttons to the existing gameplay logic. `GameUIController` now exposes events for Play, Discard, Sort by Suit, and Sort by Rank buttons, registers their `Button.onClick` listeners, and enables those buttons only while the UI state is `PlayingBlind`. `PrototypeBootstrap` subscribes to the button events and routes them through the same methods already used by the Console controls, so UI actions and keyboard actions share the same play, discard, and sorting behavior. This step only adds Play-state action button binding and does not add left status panel binding, resolution display binding, CashOut flow, Shop UI, deck stats, tooltip behavior, or new gameplay rules.

## Step 6
Bound the left Blind status panel to the current run and round state. `GameUIController` now accepts references for `BlindNameText`, `TargetScoreText`, `CurrentScoreText`, `HandTypeText`, `HandsText`, `DiscardsText`, `GoldText`, and `AnteNumberText`, and exposes `RefreshBlindStatus()` to update those labels from real gameplay data. `PrototypeBootstrap` now refreshes the left status panel through the existing `RefreshGameUI()` path, showing the current Blind display name, target score, current score, latest hand type, remaining hands, remaining discards, current Gold, and Ante number. The latest hand type resets to `None` at the start of each Blind and updates after a successful played hand. RunFailed also updates the Blind name text to `Run Failed`. This step does not add central resolution UI, Joker UI, DeckStats UI, CashOut UI, Shop UI, tooltip behavior, or new gameplay rules.

## Step 7
Bound the central play area to the latest played hand. `GameUIController` now supports `PlayedCard1-5` style card views through `RefreshPlayedCards()` and a `ResolutionInfoArea` text binding through `RefreshResolutionInfo()`. `PrototypeBootstrap` now stores the most recent played cards and score context, clears them at the start of each Blind, and refreshes the central area after a successful play. The resolution summary shows hand type, chips, mult, final score, gold reward, suit effect logs, and Joker effect logs. This step only adds central played-card and resolution display binding and does not add Joker slot UI, DeckStats UI, CashOut flow, Shop UI, tooltip behavior, or new gameplay rules.
